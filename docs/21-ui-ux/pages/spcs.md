# Snowpark Container Services — detailed screen design
...
Route: `/spcs` (prototype `#/spcs`). Persona: **FinOps analyst / data engineer**.

Goal: Pool cost, service attribution and capacity signals.

### Desktop composition

```text
+----------------------------------------------------------------------------------------------------------+
| BRIDGE DATA FINOPS  /  EXPLORE  /  Snowpark Container Services                                           |
| Acme Group [v] | PRODUCTION / organization scope | Period explicitly named per panel | USD               |
| Pool cost, service attribution and capacity signals.                                                     |
| [Demo data]  [Maturity label]  As of fixed publication  |  [Explain] [Save view] [Export CSV]            |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| COMPUTE POOL COST: $800.00 [i]  ||  COMPUTE POOLS: 1 [i]                                                 |
| Billed once per pool  ||  Observed billed pool                                                           |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| SERVICES IN POOL: 2 [i]  ||  POOL UNALLOCATED: $0.00 [i]                                                 |
| Not a cost multiplier  ||  800 minus 500 minus 300                                                       |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| SUBPAGES: pool_analytics [>]                                                                             |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| POOL COST                                                                                                |
|      [Context / evidence panel]   [Inspect details >]                                                    |
| The billed compute pool is the additive cost authority.                                                  |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| ATTRIBUTION                                                                                              |
|      [Context / evidence panel]   [Inspect details >]                                                    |
| Forecast API 500 and feature worker 300 are allocations of the same 800 USD pool.                        |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| TABLE TOOLBAR: [Search rows...] [Sort column] [Columns v] [Density v] [CSV]                              |
+----------------------------------------------------------------------------------------------------------+
+---------------------------+---------------------------+---------------------------+---------------------------+
| Compute pool              | Billed USD                | Services                  | Allocation                |
+---------------------------+---------------------------+---------------------------+---------------------------+
| pool_analytics            | 800.00                    | 2                         | Shared measured usage     |
+---------------------------+---------------------------+---------------------------+---------------------------+
+----------------------------------------------------------------------------------------------------------+
| [Previous]   Rows are labelled sample or complete in context   [Next]                                    |
| Pool and service rows cannot be summed as independent billed charges.                                    |
+----------------------------------------------------------------------------------------------------------+
```
...
| `poolcount` | Compute pools: **1** | Observed billed pool | August 2026 / synthetic; Observed |
| `servicecount` | Services in pool: **2** | Not a cost multiplier | August 2026 / synthetic; Observed |
| `spcsunallocated` | Pool unallocated: **$0.00** | 800 minus 500 minus 300 | August 2026 / synthetic; RECONCILED |
...
- **Pool cost**: The billed compute pool is the additive cost authority.
- **Attribution**: Forecast API 500 and feature worker 300 are allocations of the same 800 USD pool.
- **DataTable** columns, in order: Compute pool, Billed USD, Services, Allocation. Stable sorting, row search, column visibility, density and bounded CSV export; no automatic sum of mixed units or sample rows.
...
- **Drilldowns**: `/spcs-pool`. Use named links; never assume every row represents the same execution.
...
| Happy path | Read context → inspect Pool cost → search the table → open named evidence/drilldown → retain scope on Back. |
| Empty state | For fully covered scope with no records: “No snowpark container services for this selection.” Offer period/filter reset. A search with no matches offers Clear search. |
...
| Drilldown behavior | Breadcrumb + URL context; selected entity and period stay explicit. Related routes: spcs-pool. |
| Primary actions | Save view; Explain; search/sort/columns; CSV; Back where applicable. |
| Acceptance criteria | Fixture values equal the KPI contract; Pool and service rows cannot be summed as independent billed charges. |
...
- [ ] Open `#/spcs` directly and through the named navigation; heading and browser history agree.
- [ ] Assert every KPI above against its fixture scope; verify `spcs` explanation and status.
- [ ] Inspect exact columns: Compute pool, Billed USD, Services, Allocation. Search an existing row and a nonexistent token; sorting and exported rows agree.
...
## spcs-pool — pool_analytics

Route: `/spcs-pool` (prototype `#/spcs-pool`). Persona: **FinOps analyst / data engineer**.

Goal: Container compute pool · PRODUCTION · shared

### Desktop composition

```text
+----------------------------------------------------------------------------------------------------------+
| BRIDGE DATA FINOPS  /  EXPLORE  /  pool_analytics                                                        |
| Acme Group [v] | PRODUCTION / organization scope | Period explicitly named per panel | USD               |
| Container compute pool · PRODUCTION · shared                                                             |
| [Demo data]  [Maturity label]  As of fixed publication  |  [Explain] [Save view] [Export CSV]            |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| COMPUTE POOL COST: $800.00 [i]  ||  SERVICES IN POOL: 2 [i]                                              |
| Billed once per pool  ||  Not a cost multiplier                                                          |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| POOL UNALLOCATED: $0.00 [i]  ||  NODE-HOURS: 100 [i]                                                     |
...
| COST COMPOSITION                                                                                         |
|      [Context / evidence panel]   [Inspect details >]                                                    |
| 500 + 300 = 800. Unallocated is zero only because all synthetic pool cost is allocated.                  |
+----------------------------------------------------------------------------------------------------------+
+----------------------------------------------------------------------------------------------------------+
| TABLE TOOLBAR: [Search rows...] [Sort column] [Columns v] [Density v] [CSV]                              |
+----------------------------------------------------------------------------------------------------------+
+---------------------------+---------------------------+---------------------------+---------------------------+
| Service                   | Allocated USD             | Method                    | Confidence                |
+---------------------------+---------------------------+---------------------------+---------------------------+
| forecast_api              | 500.00                    | Measured usage            | Observed                  |
| feature_worker            | 300.00                    | Measured usage            | Observed                  |
+---------------------------+---------------------------+---------------------------+---------------------------+
+----------------------------------------------------------------------------------------------------------+
| [Previous]   Rows are labelled sample or complete in context   [Next]                                    |
| Do not multiply pool charge by service count.                                                            |
+----------------------------------------------------------------------------------------------------------+
```
...
| `servicecount` | Services in pool: **2** | Not a cost multiplier | August 2026 / synthetic; Observed |
| `spcsunallocated` | Pool unallocated: **$0.00** | 800 minus 500 minus 300 | August 2026 / synthetic; RECONCILED |
| `poolhours` | Node-hours: **100** | Observed usage; no invented unit price | August 2026 / synthetic; Observed |

Display the exact price basis, currency, publication and as-of in Explain. Monetary fixture values are illustrative and independently scoped as labelled; they are not a new production metric registry. See the [semantic contract](../../09-api/semantic-api.md).

### Components and subpage behavior

- **Capacity profile**: Node-hours and utilization are operational evidence; absent service telemetry disables finer allocation.
- **Cost composition**: 500 + 300 = 800. Unallocated is zero only because all synthetic pool cost is allocated.
...
| Persona | FinOps analyst / data engineer |
| Goal | Container compute pool · PRODUCTION · shared |
| Entry point | Parent route /spcs; deep link supported. |
| Happy path | Read context → inspect Capacity profile → search the table → open named evidence/drilldown → retain scope on Back. |
| Empty state | For fully covered scope with no records: “No pool_analytics for this selection.” Offer period/filter reset. A search with no matches offers Clear search. |
| Loading state | Keep heading/scope; skeleton occupies KPI and panel footprint. No transient zero or old-scope values. |
| Partial-data state | Show missing-source reason and affected metrics. Unknown is —, not 0. Link Data Health; suppress unsupported inference. |
| Error state | “We could not load this view.” Preserve scope; Retry; safe support reference. Form errors stay beside fields. |
| Permission-denied state | Replace content with access message; no hidden names/counts/exports. Request approved access; server remains authority. |
